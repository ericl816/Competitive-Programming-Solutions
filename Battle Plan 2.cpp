...
// We use a sparse table to check the points in the range that can be covered by a power of 2
// For each query we can build the sparse table and then output the query. We have to build the sparse table K times
// The query is a Range Maximum Query

int N, K, Q, op, l, r, SIZE;
ll a[MAXN], b[MAXN], ans[MAXN];
int sparse[MAXM][MAXN], segmax[MAXM][MAXN];
vector< pair <int, pii> > adj[MAXK]; // first = index, second.first = l, second.second = r

// @param x = number of ships that jth fighter ship can destroy
inline void Build_ST (int x) {
	...
	for (idx; idx<=N; idx++) sparse[0][idx] = N + 1;
