...
/*
 * Check for path from top left to bottom right, then apply min-cut (max-flow)
 * This works since any algorithm which solves the maximal flow, also solves the minimal cut of edges. 
 * Once you have the flows, find the residual flow graph.
 * The cost of finding the residual flow graph and running BFS is negligible, since Dinic's is O(V^2*E)
 * In this problem, you build the graph with edges that do not form a valid path (path is within the circle and/or outside rectangle)
 */
