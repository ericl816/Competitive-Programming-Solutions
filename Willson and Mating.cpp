...
struct MinCostMaxFlow {
    ...
    // Destructor for MinCostMaxFlow class
    ~MinCostMaxFlow () {
        free(dist);
        free(prev);
        g.clear();
    ...
    // After running MinCostMaxFlow, e[id].f has the flow which has passed through that edge in the optimal solution
    inline void DisplayEdges () {
        cout << "******" << "\n";
        for (int i=0; i<(int) e.size(); i++) cout << e[i].u << " " << e[i].v << " " << e[i].f << " " << e[i].c << " " << e[i].w << "\n";
        cout << "******" << "\n";
    }
    ...
    inline void PrintFlowEdges () {
        for (size_t i=0; i<e.size(); i++) {
            Edge &ed = e[i];
            // cout << ed.u << " " << ed.v << " " << ed.f << "\n";
            if (ed.f == 1 && 1 <= ed.u && ed.u <= N && N + 1 <= ed.v && ed.v <= 2 * N) cout << ed.u << " " << ed.v - N << "\n";
        }
    }
};
...
int main () {
    ...
    // flow.DisplayEdges();
    flow.PrintFlowEdges();
    // Following prints out the min cost and max flow
    // cout << ans.f << " " << ans.s << "\n";
    return 0;
}