...
inline ll Solve () {
	for (int i=N; i>=0; i--) {
		...
		for (size_t j=0; j<accus_adj[i].size(); j++) {
			...
			for (int k=W; k>=0; k--) { // # of werewolves at node i
				iswolf = notwolf = 0LL;
				for (int l=0; l<=k; l++) { // # of werewolves in children nodes of node i
		...
		for (size_t j=0; j<defend_adj[i].size(); j++) {
			...
			for (int k=W; k>=0; k--) { // # of werewolves at node i
				iswolf = notwolf = 0LL;
				for (int l=0; l<=k; l++) { // # of werewolves in children nodes of node i
					// Any robot that a werewolf defends is another robot
