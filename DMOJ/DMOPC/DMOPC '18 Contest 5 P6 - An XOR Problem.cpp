...
#define INF 9000000000000000010LL;
...
ll minxor (int e, int l1, int r1, int l2, int r2) {
	if (l1 >= r1 || l2 >= r2) return INF;
	if (e < 0) return 0LL;
	int m1 = l1, m2 = l2;
	while (m1 < r1 && ((x[m1] >> e) & 1) == 0) ++m1;
	while (m2 < r2 && ((x[m2] >> e) & 1) == 0) ++m2;
	if ((l1 < m1 && l2 < m2) || (m1 < r1 && m2 < r2)) {
		return min(minxor(e - 1, l1, m1, l2, m2), minxor(e - 1, m1, r1, m2, r2));
	}
	else {
		return min(minxor(e - 1, l1, m1, m2, r2), minxor(e - 1, m1, r1, l2, m2)) + (1LL << e);
	}
}

ll recurse (int e, int l, int r) {
	if (e < 0 || r - l <= 1) return 0LL;
	int m = l;
	while (m < r && ((x[m] >> e) & 1) == 0) ++m;
	ll cost = recurse(e - 1, l, m) + recurse(e - 1, m, r);
	if (l < m && m < r) {
		cost += minxor(e - 1, l, m, m, r) + (1LL << e);
	}
	return cost;
}
...
int main (int argc, char const *argv[]) {
	...
	for (int i=0; i<Q; i++) {
		read(c);
		for (int j=0; j<N; j++) x[j] = (x[j] % mod + c % mod) % mod;
		sort(x, x + N);
		writeln(recurse(7, 0, N));
	}
	return 0;
}
