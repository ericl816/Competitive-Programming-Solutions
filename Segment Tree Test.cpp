...
void Push_Up(int idx) {
    ...
    seg[idx].idx = 0;
    if (seg[idx].gcd == seg[idx << 1].gcd)
        seg[idx].idx += seg[idx << 1].idx;
    if (seg[idx].gcd == seg[idx << 1 | 1].gcd)
        seg[idx].idx += seg[idx << 1 | 1].idx;
}
...
int QueryMin(int idx, int l1, int r1) {
    ...
    return min(QueryMin(idx << 1, l1, mid),
               QueryMin(idx << 1 | 1, mid + 1, r1));
}
...
int QueryGCD(int idx, int l1, int r1) {
    ...
    return GCD(QueryGCD(idx << 1, l1, mid),
               QueryGCD(idx << 1 | 1, mid + 1, r1));
}
...
int QueryNum(int idx, int l1, int r1, int val) {
    ...
    return QueryNum(idx << 1, l1, mid, val) +
           QueryNum(idx << 1 | 1, mid + 1, r1, val);
}
