...
// Essentially, we want to build a MST with no bridges
...
inline void TarjansDFS (int node, int prev, int bit) {
	...
	for (size_t i=0; i<adj[node].size(); i++) {
		...
		if (!vis[next.s.f]) {
			...
			// Found a bridge
			if (low[next.s.f] == DFN[next.s.f] || low[next.s.f] > DFN[node]) {
				// cout << node << " and " << next.s.f << " are part of a bridge" << "\n";
				bridges++;
			}
		}
...
int main () {
	...
	while (1) {
		...
		for (int i=0; i<1 << m; i++) {
			...
			// cout << bit << endl;
			for (int j=0; j<n; j++) if (!vis[j]) TarjansDFS(j, -1, bit);
			if (!bridges) {
				tot = 0;
				for (int j=0; j<m; j++) {
					if (!(i & (1 << j))) continue;
